...
async fn test_password_reset_email_service_integration() {
    ...
    let result = email_manager.send_password_reset_email(
        "test@example.com".to_string(),
        "test_reset_token_123".to_string(),
    ).await;

    assert!(result.is_ok(), "Password reset email should send successfully");
}
...
async fn test_password_changed_notification_service() {
    ...
    let result = email_manager.send_password_changed_notification(
        "test@example.com".to_string(),
    ).await;

    assert!(result.is_ok(), "Password changed notification should send successfully");
}
...
async fn test_password_reset_rate_limiting() {
    ...
    for i in 1..=3 {
        let remaining = storage.check_password_reset_rate_limit(test_email).await
