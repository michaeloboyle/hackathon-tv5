...
async fn test_password_reset_invalidates_sessions(pool: PgPool) -> Result<()> {
    ...
    let session1 = session_manager
        .create_session(user_id.to_string(), "jti1".to_string(), Some("device1".to_string()))
        .await?;
    let session2 = session_manager
        .create_session(user_id.to_string(), "jti2".to_string(), Some("device2".to_string()))
        .await?;
    ...
    assert!(session_manager.get_session(&session1.session_id).await.is_ok());
    assert!(session_manager.get_session(&session2.session_id).await.is_ok());
    assert!(session_manager.get_session(&session3.session_id).await.is_ok());

    // Create token families (refresh tokens)
    let family1 = token_family_manager.create_family(user_id).await?;
    token_family_manager.add_token_to_family(family1, "token1").await?;

    let family2 = token_family_manager.create_family(user_id).await?;
    token_family_manager.add_token_to_family(family2, "token2").await?;
    ...
    storage.store_password_reset_token(&reset_token.token, &reset_token).await?;
    ...
    assert!(session_manager.get_session(&session1.session_id).await.is_err());
    assert!(session_manager.get_session(&session2.session_id).await.is_err());
    assert!(session_manager.get_session(&session3.session_id).await.is_err());
...
async fn test_revoke_all_user_tokens() -> Result<()> {
    ...
    token_family_manager.add_token_to_family(other_family, "other_token").await?;

    // Revoke all tokens for target user
    let revoked = token_family_manager.revoke_all_user_tokens(&user_id).await?;
    assert_eq!(revoked, 2);
