...
async fn test_cache_invalidation_pattern() {
    ...
    let _: () = conn.set("cache:GET:/api/content/1", &cache_value).await.unwrap();
    let _: () = conn.set("cache:GET:/api/content/2", &cache_value).await.unwrap();
    let _: () = conn.set("cache:GET:/api/other/1", &cache_value).await.unwrap();

    // Invalidate pattern
    let count = cache_middleware.invalidate_pattern("GET:/api/content/*").await.unwrap();
    assert_eq!(count, 2);
