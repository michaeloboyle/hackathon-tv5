...
fn create_test_content_with_eidr(title: &str, eidr: &str) -> CanonicalContent {
    ...
    content.external_ids.insert("eidr".to_string(), eidr.to_string());
    content
}
...
async fn test_upsert_new_content() {
    ...
    // Insert new content
    let content_id = repo.upsert(&content).await.expect("Failed to upsert content");
    ...
    // Verify external IDs
    let ext_id: (Option<String>, Option<String>) = sqlx::query_as(
        "SELECT imdb_id, tmdb_id FROM external_ids WHERE content_id = $1"
    )
    .bind(content_id)
    .fetch_one(&pool)
    .await
    ...
    // Verify genres
    let genre_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM content_genres WHERE content_id = $1"
    )
    .bind(content_id)
    .fetch_one(&pool)
    .await
    ...
    let avail_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM platform_availability WHERE content_id = $1"
    )
    .bind(content_id)
    .fetch_one(&pool)
    .await
...
async fn test_upsert_existing_content_by_imdb() {
    ...
    content1.external_ids.insert("imdb".to_string(), "tt9999999".to_string());

    // Insert first version
    let content_id_1 = repo.upsert(&content1).await.expect("Failed to insert first version");
    ...
    content2.external_ids.insert("imdb".to_string(), "tt9999999".to_string());
    content2.runtime_minutes = Some(150);

    // Upsert should update, not create new record
    let content_id_2 = repo.upsert(&content2).await.expect("Failed to upsert second version");
    ...
    let result: (String, i32) = sqlx::query_as(
        "SELECT c.title, c.runtime_minutes FROM content c WHERE c.id = $1"
    )
    .bind(content_id_2)
    .fetch_one(&pool)
    .await
    ...
    let count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM external_ids WHERE imdb_id = $1"
    )
    .bind("tt9999999")
    .fetch_one(&pool)
    .await
...
async fn test_upsert_existing_content_by_eidr() {
    ...
    let content_id_1 = repo.upsert(&content1).await.expect("Failed to insert with EIDR");
    ...
    let content_id_2 = repo.upsert(&content2).await.expect("Failed to upsert with EIDR");

    // Should match by EIDR and update
    assert_eq!(content_id_1, content_id_2);

    let overview: String = sqlx::query_scalar(
        "SELECT overview FROM content WHERE id = $1"
    )
    .bind(content_id_2)
    .fetch_one(&pool)
    .await
...
async fn test_upsert_existing_content_by_title_and_year() {
    ...
    let runtime: i32 = sqlx::query_scalar(
        "SELECT runtime_minutes FROM content WHERE id = $1"
    )
    .bind(content_id_2)
    .fetch_one(&pool)
    .await
...
async fn test_upsert_batch_atomic() {
    ...
    // Upsert batch
    let content_ids = repo.upsert_batch(&contents).await.expect("Failed to upsert batch");
...
async fn test_upsert_batch_large() {
    ...
    for i in 1..=25 {
        ...
        content.external_ids.insert("imdb".to_string(), format!("tt{:07}", 8000000 + i));
        contents.push(content);
    }

    // Upsert batch
    let content_ids = repo.upsert_batch(&contents).await.expect("Failed to upsert large batch");

    assert_eq!(content_ids.len(), 25);

    // Verify all were inserted
    let count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM content WHERE id = ANY($1)"
    )
    .bind(&content_ids)
    .fetch_one(&pool)
    .await
...
async fn test_upsert_batch_with_updates() {
    ...
    let initial_ids = repo.upsert_batch(&initial_contents).await.expect("Failed to insert initial batch");
    ...
    let updated_ids = repo.upsert_batch(&updated_contents).await.expect("Failed to upsert updated batch");

    // IDs should match (updates, not inserts)
    assert_eq!(initial_ids, updated_ids);

    // Verify updates
    let runtime1: i32 = sqlx::query_scalar(
        "SELECT runtime_minutes FROM content WHERE id = $1"
    )
    .bind(updated_ids[0])
    .fetch_one(&pool)
    ...
    let count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM external_ids WHERE eidr_id IN ($1, $2)"
    )
...
async fn test_genre_updates() {
    ...
    content1.external_ids.insert("imdb".to_string(), "tt7777777".to_string());
    ...
    content2.external_ids.insert("imdb".to_string(), "tt7777777".to_string());
    content2.genres = vec!["Drama".to_string(), "Romance".to_string(), "Comedy".to_string()];

    repo.upsert(&content2).await.expect("Failed to update genres");

    // Verify updated genres
    let updated_genre_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM content_genres WHERE content_id = $1"
    )
    .bind(content_id)
    .fetch_one(&pool)
    ...
    let genres: Vec<String> = sqlx::query_scalar(
        "SELECT genre FROM content_genres WHERE content_id = $1 ORDER BY genre"
    )
    .bind(content_id)
    .fetch_all(&pool)
    .await
...
async fn test_platform_availability_updates() {
    ...
    content1.external_ids.insert("imdb".to_string(), "tt6666666".to_string());
    ...
    content2.external_ids.insert("imdb".to_string(), "tt6666666".to_string());
    content2.availability.regions = vec!["US".to_string(), "CA".to_string(), "UK".to_string()];
    content2.availability.purchase_price = Some(12.99);

    repo.upsert(&content2).await.expect("Failed to update availability");

    // Verify updated availability
    let updated_avail_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM platform_availability WHERE content_id = $1"
    )
    .bind(content_id)
    .fetch_one(&pool)
